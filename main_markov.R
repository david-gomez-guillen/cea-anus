...
# 
# write.xlsx(summary.df, 'output/results/summary_anus_new_long.xlsx')
...
# 
var.names <- names(summary.df)[2:34]
...
new.summary.df <- data.frame(strategy=c('conventional', rep(strat.names[2:8], 2), 'no_intervention'),
                             ascus.diff=c('-', rep('no.ascus.diff', 7), rep('ascus.diff', 7), 'no_intervention'))
for(var.name in var.names) {
  for(treatment in treatment.names) {
    var.summary.df <- summary.df[summary.df$strat.treatment==treatment,]
    var.summary.df.0 <- var.summary.df[var.summary.df$ascus.diff=='conventional',][[var.name]]
    var.summary.df.1 <- var.summary.df[var.summary.df$ascus.diff=='no.ascus.diff',][[var.name]]
    var.summary.df.2 <- var.summary.df[var.summary.df$ascus.diff=='ascus.diff',][[var.name]]
    var.summary.df.3 <- var.summary.df[var.summary.df$ascus.diff=='no_intervention',][[var.name]]
    if (length(var.summary.df.3) == 0) var.summary.df.3 <- NA
    
    new.summary.df[paste0(var.name, '_', treatment)] <- c(var.summary.df.0,
                 var.summary.df.1,
                 var.summary.df.2,
                 var.summary.df.3
                )
...
View(new.summary.df)
write.xlsx(new.summary.df, 'output/results/summary_anus_new_v4_wide.xlsx')
