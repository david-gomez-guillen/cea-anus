plotMortality <- function(..., current=NULL, labels=NULL)
{
  ...
  CCdeaths1 <- attr(dots[[1]], "ACdeaths")
  ...
  print(cbind(as.numeric(totMort1*100000)))
  par(xpd=TRUE)
  colorets <- "#000000"
  plot(age, totMort1*100000, pch = 20, xlab = "Age", ylab = "Mortality",
       main = "AC mortality by age", axes = FALSE, type = "l", col=colorets)
  axis(2)
  axis(1)
  if (length(dots)>1)
  {
    ...
    {
      CCdeaths1 <- attr(dots[[j]], "ACdeaths")
      CCdeathsMean1 <- apply(CCdeaths1, 2, mean)
      ...
      lines(age, totMort1*100000, col=colorets[j])
    }
  }
  if (!is.null(current)) 
  {
  ...
  return(list(data.frame(age=seq(20:84), val=cbind(as.numeric(apply(CCdeaths1, 2, mean)))), 
              data.frame(age=seq(20:84), val=cbind(as.numeric(totMort1*100000)))))
}