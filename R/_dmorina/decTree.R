...
simCohort <- function(M, period, nsim, seed=NULL, disc=3)
{
  ...
  for (i in 1:nsim)
  {
    ...
    qalys1[i] <- table(rcyto)[1]*period*1 +  # Benign cytology
      table(ranos)[1]*period*0.97 +          # Normal anoscopy
      table(rbiops)[1]*period*0.87 +         # LSIL
      table(rbiops)[2]*period*0.76 +         # HSIL
      table(rbiops)[3]*period*0.67           # IC
