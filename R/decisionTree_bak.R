...
Node <- setRefClass('Node',
                    fields=c('name', 'cost', 'eff', 'childrenProbs', 'children'),
                    methods=list(
                      ...
                      parseProbs = function(context=list()) {
                        actualChildrenProbs <- childrenProbs
                      ...
                      getExpectedMeasure = function(measure, context=list()) {
                        m <- parseNodeValue(get(measure), context)
                        if (length(m) == 0 || is.na(m)) {
                          # warning(paste0('"', measure, '" does not exist for node "', name, '"; assuming 0'))
                          currentMeasure <- 0
                        }
                        else {
                          currentMeasure <- m
                      ...
                      getExpectedCost = function(context=list()) {
                        return(getExpectedMeasure('cost', context))
                      },
                      ...
                      runIteration = function(context=list()) {
                        if (length(children) == 0) {
                          ...
                          probs <- parseProbs(context)
                          randomChildIndex <- sample(length(probs), size=1, prob=probs)
                      ...
                      toString = function(level = 0, prob = NULL) {
                        ...
                        info <- c()
                        if (!is.null(prob)) {
                          info <- c(info, paste0('p = ', prob))
                        }
                        if (!is.null(cost)) {
                          info <- c(info, paste0('cost = ', cost))
                        }
                        if (!is.null(eff)) {
                          info <- c(info, paste0('eff = ', eff))
                        }
                        if (length(info) > 0) {
                          displayedTree <- paste0(displayedTree, ' (', paste0(info, collapse = ', '), ')')
                        }
...
setwd('~/projects/modeling/anus/R/trees')
conventional <- parseJSONDecisionTree(paste0('conventional.json'))

results <- c()
for(i in seq(100)) {
  endNode <- conventional$runIteration(context=list(
    ...
    dist_aar_results = rdirichlet(N, c(6, 3, 1)),
    
    c_cytology=runif(N,100,200),
    c_followup=runif(N,300,400),
    c_aar=runif(N,5000,7000),
    c_surgery=runif(N,10000,15000),
    c_cytology_hpvla=runif(N,200,400),
    c_cytology_hpvhc=runif(N,100,200),
    c_cytology_arnme6e7=runif(N,300,400),
    
    u_cyto_benign=runif(N,.95,1),
    u_no_hsil=runif(N,.95,1),
    u_regression=runif(N,.8,.85),
    u_surgery=runif(N,.5,.8),
    u_aar=runif(N,.8,.9)
  ))
  results <- append(results, endNode$name)
}
