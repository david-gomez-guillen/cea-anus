plotIncidence <- function(..., current=NULL, labels=NULL)
{
  ...
  par(xpd=TRUE)
  colorets <- "#000000"
  plot(age, totInc1*100000, pch = 20, xlab = "Age", ylab = "Incidence",
       main = "AC Incidence by age", axes = FALSE, type = "l", col=colorets)
  axis(2)
  axis(1)
  if (length(dots)>1)
  {
    ...
    for (j in 2:length(dots))
    {
      ...
      lines(age, totInc1*100000, col=colorets[j])
    }
  }
  if (!is.null(current)) 
  {
  ...
  return(list(data.frame(age=seq(20:84), val=cbind(as.numeric(apply(newCases1, 2, mean)))), 
              data.frame(age=seq(20:84), val=cbind(as.numeric(totInc1*100000)))))
}